...
- 카드: 각 노드 박스는 캔버스 너비의 약 20~23%를 넘지 않게 하고, 높이도 현재보다 더 얕게 만들어 답답함을 줄일 것
- 카드: 외곽선은 1.0~1.5px 수준의 얇은 회색 선으로 처리하고, 그림자는 거의 느껴지지 않을 정도로 최소화할 것
- 카드: 회색 선은 카드 외곽선에만 제한적으로 허용되며, 카드 밖 연결선이나 방향선으로 확장되지 않게 할 것
- 카드: 각 카드 내부 패딩은 얇게 유지하고, 텍스트 블록이 카드 중앙에 작게 앉도록 할 것
- 카드: 현재 결과물보다 카드 전체 면적을 최소 20% 더 줄여, 카드보다 배경 여백이 더 넓게 보이게 할 것
...
- 아이콘: 각 카드 좌상단 또는 타이틀 옆에 작게 두고, 카드 안에서 텍스트보다 시각적 비중이 커지지 않게 할 것
- 아이콘: 아이콘 크기는 카드 높이의 10~12%를 넘지 않게 할 것
...
- 폰트/텍스트: 현재 결과물보다 title은 최소 30%, bullet은 최소 40% 더 작게 그릴 것
...
- 폰트/텍스트: bullet은 가능한 한 한 줄에 짧게 넣고, 자동 줄바꿈으로 박스를 채우지 말 것
...
- 돈 흐름은 초록색 + 달러 마커
- 서비스 흐름은 주황색 + 원형 마커
- 회색, 검정, 무채색 화살표를 별도로 추가하지 말 것
- 모든 화살표는 반드시 위 3종 중 하나의 색상 체계를 따라야 하며, 범례 없는 보조 화살표는 금지
- 위에서 정의한 정보/돈/서비스 흐름 외에 상단 행, 하단 행, 좌우 행을 단순 연결하는 회색 내비게이션 화살표를 절대 만들지 말 것
- 글자는 모두 한국어
- 전체 화면 비율은 가로형 16:10에 가깝게
- 캔버스 외곽에도 충분한 여백을 두고, 다이어그램이 화면 가장자리까지 꽉 차게 확장되지 않게 할 것
- 전체 다이어그램은 캔버스 중앙 80% 영역 안에서만 구성하고, 바깥 20%는 breathing room으로 남겨둘 것
- 하단 중앙에 범례를 반드시 포함
...
- 3x3 노드, 화살표 라벨, 범례 외의 추가 텍스트 블록을 절대 넣지 말 것
- BMC 본문, BMC 요약, 설명문 문단, 보고서형 부연 텍스트를 이미지에 따로 출력하지 말 것
- 노드 밖에 문장형 설명 텍스트, 캡션, 주석 박스, 본문 패널을 추가하지 말 것
...
- 각 열 사이 가로 간격과 각 행 사이 세로 간격을 크게 유지하여, 카드보다 화살표가 더 길게 보이도록 할 것
- 중앙 노드 주변에는 여백을 넓게 비워 두어 정보/돈/서비스 화살표가 겹치지 않고 호흡감 있게 보이도록 할 것
- 중앙 카드와 주변 카드 사이 간격은 최소 카드 너비의 0.5배 이상으로 유지해 화살표 라벨이 들어갈 빈 공간을 확보할 것
...
- 도해도 자체만 보여주고, BMC 내용을 텍스트로 재출력하는 레이아웃은 금지할 것
...
- 화살표 선은 얇지만 또렷하게, 너무 두껍지 않게 그리고, 각 라벨은 화살표 중앙 근처의 빈 공간에 배치할 것
- 화살표 라벨은 카드 경계에 붙이지 말고, 선을 따라 읽기 쉬운 중간 지점에 두어 시선 흐름을 방해하지 않게 할 것
- 화살표 선 두께는 카드 외곽선보다 약간 두껍되, 텍스트보다 시각적으로 무거워 보이지 않게 할 것
- 보조 연결선처럼 보이는 회색 화살표, 장식용 선, 의미 없는 직선 연결은 절대 넣지 말 것
- Problem -> Target -> Channel 또는 Value Proposition -> 기업명 -> Moat 같은 템플릿형 회색 진행선은 절대 넣지 말 것
- 행과 행 사이의 회색 연결선, 단계 진행선, 개념 연결선은 모두 금지하고, 실제 flow_type이 검증된 화살표만 그릴 것
...
- 3x3 노드, 화살표 라벨, 범례 외의 텍스트 박스가 보이면 실패다
- BMC 본문이 통째로 이미지에 재출력되거나, 설명문 문단이 별도 텍스트로 보이면 실패다
- 화살표가 적거나 짧아서 작동 구조가 안 보이면 실패다
- 카드가 너무 커서 3x3 사이 간격이 좁아지면 실패다
- 텍스트가 커서 여백이 사라지면 실패다
- 중앙과 주변 카드 사이가 좁아 화살표가 짧아 보이면 실패다
- 다이어그램이 전체적으로 꽉 차 보여 숨막히면 실패다
- 아이콘이 크거나 카드가 두꺼워서 올드한 PPT 도식처럼 보이면 실패다
...
- 모든 화살표는 반드시 초록/파랑/주황 중 하나여야 하며, 회색 화살표가 하나라도 있으면 실패다
- 카드 외곽선 외에 회색 선이 연결선처럼 보이면 실패다
