...
- 폰트/텍스트: 모든 제목, 부제, bullet 텍스트를 현재 일반 다이어그램보다 명확하게 더 작게 그릴 것
- 폰트/텍스트: 체감상 현재 크기의 정확히 약 50% 수준으로 축소해, 카드 내부 여백이 넉넉하게 보이도록 할 것
- 폰트/텍스트: title도 크게 두지 말고 compact bold 스타일의 작은 제목으로 처리할 것
- 폰트/텍스트: bullet 텍스트는 매우 작은 크기로 통일하되 읽을 수는 있게 유지할 것
- 폰트/텍스트: 텍스트가 카드 면적의 대부분을 차지하면 안 되며, 카드 안에 빈 여백이 충분히 남아야 함
