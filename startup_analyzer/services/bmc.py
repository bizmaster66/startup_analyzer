...
- JSON ONLY
- 모든 일반 라벨은 한국어로 작성
- 우선순위는 사실 정보와 기존 기업 분석 JSON의 설명문 맥락을 먼저 따르고, business_model_canvas는 그 구조화 결과로 작성할 것
- 즉 설명문을 BMC에 억지로 맞추기보다, 설명문에 나온 실제 문제/서비스/고객/채널/파트너를 최대한 보존해 BMC로 구조화할 것
- business_model_canvas의 각 항목은 짧고 구체적인 한국어 명사구로 작성
- 추정이 필요하면 보수적으로 작성
- 과도하게 일반적인 표현 금지
- "대기업", "중견기업", "AI 기술 협력사", "플랫폼", "서비스" 같은 generic 표현만 단독으로 쓰지 말 것
- 파트너와 채널은 가능하면 실제 서비스명, 실제 유통/영업 채널, 실제 파트너사명 또는 역할이 드러나는 구체 표현으로 작성할 것
- 예: "올리브영 H&B", "공식 자사몰", "스토리네이션", "OEM·ODM 제조사", "IP 유통 파트너"
- top_layer, left_actors, right_actors는 다이어그램 노드용 짧은 명사구 사용
- middle_layer는 가능한 한 실제 서비스명/제품명/플랫폼명을 우선 사용
- 예: "스토리네이션", "올웨이즈", "세이프엑스", "바리스브루"
